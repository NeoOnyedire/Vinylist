...
/**
 * Client Credentials token - used for catalog search (no specific user needed).
...
async function getAlbum(albumId) {
  ...
  if (!resp.ok) {
    throw new Error(`Spotify album lookup failed: ${resp.status}`);
  }

  const data = await resp.json();
  return formatAlbum(data);
}
